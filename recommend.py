...
def find_closest(location, centroids):
    ...
    # BEGIN Question 3
    "*** REPLACE THIS LINE ***"
    return min([cent for cent in centroids], key = lambda x: distance(x, location))
    # END Question 3
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** REPLACE THIS LINE ***"
    best = [find_predictor(user, reviewed, f) for f in feature_fns]
    predict = {predictor: r_squared for predictor, r_squared in best}
...
def rate_all(user, restaurants, feature_fns):
    ...
    # BEGIN Question 9
    "*** REPLACE THIS LINE ***"
    pred_rest_name = [restaurant_name(restaurant) for restaurant in restaurants]
    pred_rest_review = [predictor(restaurant) for restaurant in restaurants]
    pred_rate = {name: rating for name, rating in zip(pred_rest_name, pred_rest_review)}
